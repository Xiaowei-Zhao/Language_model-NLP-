...
# 1.3 To explore results for smoothed LMs
...
def smoothed_perplexity():
    en=LangModel("./langID_data_tok/HW1english.txt")
    test=corpus_reader("test.txt", lexicon=en.lexicon)               
    print("The smoothed perplexity for English(Bigram) is : {}".format(en.perplexity_smoothed_bigram_probability(test))) 
    
    fr=LangModel("./langID_data_tok/HW1french.txt")
    test=corpus_reader("test.txt", lexicon=fr.lexicon)
    print("The smoothed perplexity for French(Bigram) is : {}".format(fr.perplexity_smoothed_bigram_probability(test)))
    
    ge=LangModel("./langID_data_tok/HW1german.txt")
    test=corpus_reader("test.txt", lexicon=ge.lexicon)
    print("The smoothed perplexity for German(Bigram) is : {}".format(ge.perplexity_smoothed_bigram_probability(test)))
    ...
    with open(testfile, 'r',encoding='latin-1') as f:
        for i in f:
            ...
            with open(test_file, 'w',encoding='latin-1') as output:
                output.write(line)
            test=corpus_reader("test.txt", lexicon=en.lexicon)               
            en2=en.perplexity_smoothed_bigram_probability(test)  
    
            test=corpus_reader("test.txt", lexicon=fr.lexicon)
            fr2=fr.perplexity_smoothed_bigram_probability(test)
            
            test=corpus_reader("test.txt", lexicon=ge.lexicon)
            ge2=ge.perplexity_smoothed_bigram_probability(test)
...
def cal_smoothed_accuracy(result):
...
smoothed_perplexity() 
result2=smoothed_accuracy() 
cal_smoothed_accuracy(result2)  
